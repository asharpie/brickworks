...
// libSQL returns rows as array-like Row objects plus a columns[] array.
// Convert to plain objects and coerce BigInt → Number so res.json() doesn't
// choke. Defensive against unexpected shapes (null rows, stray column entries).
function toObjects(rs) {
  if (!rs || !Array.isArray(rs.rows)) return [];
  const cols = Array.isArray(rs.columns) ? rs.columns : [];
  return rs.rows.map((row) => {
    ...
    for (let i = 0; i < cols.length; i++) {
      ...
      const v = row[i];
      obj[name] = typeof v === 'bigint' ? Number(v) : v;
    }
    return obj;
  });
}
